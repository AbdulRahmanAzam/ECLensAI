...
const envSchema = z.object({
  ...
  /** Where uploaded portfolio files are kept. Relative paths resolve from apps/api. */
  ...
  /**
   * `sync` runs jobs inside the API process and needs no broker — this is the
   * documented development fallback so the demo is never blocked by Redis.
   * `bullmq` requires REDIS_URL.
   */
  JOB_QUEUE_DRIVER: z.enum(['sync', 'bullmq']).default('sync'),
...
/** BullMQ is used only when it is both requested and reachable; otherwise fall back. */
export const resolvedJobQueueDriver: 'sync' | 'bullmq' =
  env.JOB_QUEUE_DRIVER === 'bullmq' && env.REDIS_URL ? 'bullmq' : 'sync';
