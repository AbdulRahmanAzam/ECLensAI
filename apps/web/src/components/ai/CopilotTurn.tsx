...
export function CopilotTurn({ turn, threadId, onAsk, isLatest, disabled }: CopilotTurnProps) {
  ...
  const feedback = useMutation({
    mutationFn: (vote: 'UP' | 'DOWN') =>
      api.ai.feedback(threadId, turn.id, {
        vote,
    ...
    onError: () =>
      push(
        ...
        'The vote could not be saved. The answer itself is unaffected.',
      ),
  });
  ...
  if (turn.role === 'user') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] rounded-2xl rounded-br-md bg-brand px-4 py-2.5 text-sm leading-relaxed text-white shadow-xs">
  ...
  const copy = async () => {
    ...
    push(
      ...
      copied ? 'Answer copied' : 'Copy failed',
      copied ? undefined : 'This browser would not write to the clipboard.',
    );
  };
      ...
      <div className="min-w-0 max-w-[86%] flex-1 space-y-2.5">
        <div
          className={cn(
            ...
            turn.degraded ? 'border-amber-200 bg-amber-50' : 'border-line bg-surface-2',
          )}
        >
          <div className="mb-2 flex flex-wrap items-center gap-1.5">
            {answer ? <AiConfidenceBadge label={answer.confidenceLabel} /> : null}
            {turn.degraded ? (
              <Badge tone="warning">Demo explanation — no model answered</Badge>
            ) : null}
            {turn.status === 'SCHEMA_REPAIRED' ? (
              <Badge tone="warning">corrected on retry</Badge>
            ) : null}
            {turn.model ? (
              <span className="font-mono text-[10px] text-slate-400">{turn.model}</span>
            ) : null}
        ...
        <details className="rounded-lg border border-line bg-surface">
          <summary className="cursor-pointer px-3 py-1.5 text-2xs font-medium uppercase tracking-wider text-slate-500">
            What was consulted
          </summary>
        ...
        {noteOpen ? (
          <div className="flex items-center gap-2">
            <input
              ...
              maxLength={1000}
              className="h-8 flex-1 rounded-lg border border-slate-300 bg-surface px-2.5 text-xs text-slate-900 focus:border-navy-500 focus:outline-none focus:ring-2 focus:ring-navy-500"
            />
            <Button
              size="sm"
              variant="secondary"
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            {answer.suggestedQuestions.map((question) => (
              <button
                ...
                className="rounded-full border border-navy-200 bg-surface px-2.5 py-1 text-2xs text-navy-800 transition-colors hover:border-navy-400 hover:bg-navy-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {question}
              </button>
            ))}
