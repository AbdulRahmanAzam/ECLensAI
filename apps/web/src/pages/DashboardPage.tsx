...
import { AlertTriangle, Banknote, Clock, GitCompare, Layers, Percent, ShieldAlert, Target, Wallet } from 'lucide-react';
...
const FILTER_KEYS = ['snapshotId', 'segment', 'productType', 'region', 'industry', 'rating', 'stage'] as const;
...
export function DashboardPage() {
  ...
  const fullFilter: AnalyticsFilter = { snapshotId, segment, productType, region, industry, rating, stage };
  ...
  const driverColumns = [
    ...
    driverColumn.accessor('stage', { header: 'Stage', cell: (info) => <StageBadge stage={info.getValue()} /> }),
    driverColumn.accessor('lossAllowance', {
      header: 'Allowance',
      cell: (info) => <span className="font-medium text-red-700">{moneyString(info.getValue(), { compact: true })}</span>,
    }),
  ...
  const activePeriod = filtersQuery.data?.periods.find((period) => period.snapshotId === (snapshotId ?? filtersQuery.data?.periods[0]?.snapshotId));
    ...
    <div>
      ...
      <Card className="mb-4">
        <CardContent className="flex flex-wrap items-end gap-3 py-3">
          ...
          <Select label="Segment" value={segment ?? ''} onChange={(event) => setParam('segment', event.target.value || undefined)} className="w-36">
          ...
          <Select label="Product" value={productType ?? ''} onChange={(event) => setParam('productType', event.target.value || undefined)} className="w-36">
          ...
          <Select label="Region" value={region ?? ''} onChange={(event) => setParam('region', event.target.value || undefined)} className="w-36">
          ...
          <Select label="Industry" value={industry ?? ''} onChange={(event) => setParam('industry', event.target.value || undefined)} className="w-36">
          ...
          <Select label="Rating" value={rating ?? ''} onChange={(event) => setParam('rating', event.target.value || undefined)} className="w-28">
          ...
          <Select label="Stage" value={stageParam ?? ''} onChange={(event) => setParam('stage', event.target.value || undefined)} className="w-28">
          ...
          {activePeriod ? (
            <span className="ml-auto flex items-center gap-1.5 text-[11px] text-slate-400">
              ...
              {activePeriod.hasResults ? `Calculated as of ${formatDate(activePeriod.reportingDate)}` : 'No completed run for this period'}
            </span>
          ) : null}
        </CardContent>
        {chips.length > 0 ? (
          <CardContent className="border-t border-slate-100 py-2.5">
            <FilterChips chips={chips} onRemove={(key) => setParam(key as FilterKey, undefined)} onClearAll={clearAll} />
          </CardContent>
        ) : null}
      </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <MetricCard
          ...
          hint={movementQuery.data ? `vs. ${formatDate(movementQuery.data.from.reportingDate)}` : 'Scenario-weighted'}
          delta={movementDelta}
        ...
        />
        <Card className="p-4">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Stage share (gross)</p>
          {isLoading || !summary ? (
            ...
            <div className="mt-2 space-y-1">
              {summary.stages.map((row) => (
                <div key={row.stage} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-semibold tabular-nums text-navy-950">{percentString(row.shareOfGrossPercent, 0)}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader title="Allowance movement" description="What moved the allowance between the two most recent reporting periods" />
          <CardContent>
            <MovementBridgeChart bridge={movementQuery.data} loading={movementQuery.isLoading} />
          </CardContent>
        </Card>
        <Card>
          <CardHeader title="ECL and coverage trend" description="Allowance and coverage ratio across every reporting period" />
          <CardContent>
            {trendQuery.isLoading || trendPoints.length === 0 ? (
              ...
              <>
                <p className="sr-only">
                  {trendPoints
                    .map((point) => `${point.label}: allowance ${point.allowanceText}, coverage ${point.coverageRatioText}`)
                    .join('. ')}
                </p>
                  ...
                  <AreaChart data={trendPoints} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
                    <defs>
                      <linearGradient id="allowanceFill" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#2c4a6f" stopOpacity={0.25} />
                        <stop offset="100%" stopColor="#2c4a6f" stopOpacity={0.02} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                    <XAxis
                      dataKey="reportingDate"
                      tickFormatter={(value: string) => new Date(value).toLocaleDateString('en-GB', { month: 'short', timeZone: 'UTC' })}
                    ...
                    />
                    <ChartTooltip
                      formatter={(_value: number, name: string, item: { payload?: (typeof trendPoints)[number] }) =>
                        name === 'allowance'
                          ? [moneyString(item.payload?.allowanceText), 'Allowance']
                          : [formatDecimalPercent(item.payload?.coverageRatioText), 'Coverage ratio']
                      }
                      labelFormatter={(label: string) => formatDate(String(label))}
                      contentStyle={{ fontSize: 12, borderRadius: 8, borderColor: '#e2e8f0' }}
                    />
                    <Area yAxisId="allowance" type="monotone" dataKey="allowance" stroke="#2c4a6f" strokeWidth={2} fill="url(#allowanceFill)" />
                    <Line yAxisId="coverage" type="monotone" dataKey="coverageRatio" stroke="#dc2626" strokeWidth={2} dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
                <div className="mt-1 flex gap-4 text-[11px] text-slate-500">
                  <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-navy-700" /> Allowance</span>
                  <span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-red-600" /> Coverage ratio</span>
                </div>
              </>
            )}
          </CardContent>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader title="Stage distribution" description="EAD and ECL allowance by IFRS 9 stage" />
          <CardContent className="space-y-4">
            {isLoading || !summary ? (
              ...
              summary.stages.map((row) => {
                const barColor = row.stage === 1 ? 'bg-teal-500' : row.stage === 2 ? 'bg-amber-500' : 'bg-red-500';
                const width = Math.max(decimalStringToNumber(row.shareOfGrossPercent ?? '0'), row.exposureCount > 0 ? 1 : 0);
                return (
                  <div key={row.stage}>
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-slate-500">{percentString(row.coverageRatio, 1)} coverage</span>
                    </div>
            ...
            <p className="border-t border-slate-100 pt-3 text-[11px] leading-relaxed text-slate-500">
              Stage 1 recognises 12-month ECL. Stages 2 and 3 recognise lifetime ECL over the remaining contractual
              horizon; Stage 3 net carrying amount is gross carrying amount less the allowance.
            </p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader title="Stage migration" description="Movement between the two most recent reporting periods" icon={<GitCompare className="h-4 w-4 text-navy-500" />} />
          <CardContent>
            <StageMigrationMatrix migration={migrationQuery.data} loading={migrationQuery.isLoading} />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader title="Scenario contribution" description="Base, upside and downside impact on the reported allowance" />
          <CardContent>
            {scenariosQuery.isError ? (
              <EmptyState title="No scenario split available" description="This period has no completed run to split by scenario." />
            ) : (
              <ScenarioComparisonChart scenarios={scenariosQuery.data} loading={scenariosQuery.isLoading} />
            )}
          </CardContent>
        </Card>
        {summary?.run ? (
          <div data-tour="tour-executive-commentary">
            <ExecutiveCommentaryPanel runId={summary.run.publicId} runLabel={summary.run.publicId} />
          </div>
        ) : (
          <Card>
            <CardHeader title="Executive commentary" />
            <CardContent>
              <EmptyState title="No completed run" description="Executive commentary needs a completed run to summarise." />
            </CardContent>
          </Card>
        )}
      </div>
      ...
      {/* Concentration -------------------------------------------------------- */}
      <Card className="mt-4">
        <CardHeader title="Risk concentration" description="Where the allowance is concentrated, ranked largest first" />
        <CardContent>
              ...
              tabs={concentrationQuery.data.slices.map((slice) => ({
                ...
                content: (
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {[...slice.buckets, ...(slice.otherBucket ? [slice.otherBucket] : [])].map((bucket) => (
                        <tr key={bucket.key} className="border-b border-slate-100 last:border-b-0">
                          ...
                          <td className="py-2 text-right tabular-nums">{moneyString(bucket.grossCarryingAmount, { compact: true })}</td>
                          <td className="py-2 text-right tabular-nums text-red-700">{moneyString(bucket.lossAllowance, { compact: true })}</td>
                          <td className="py-2 text-right tabular-nums">{percentString(bucket.shareOfAllowancePercent, 1)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader
            ...
            actions={
              <Link to="/portfolio">
                <Button variant="ghost" size="sm">Open portfolio</Button>
              </Link>
            }
          />
          <DataTable
        ...
        <Card>
          <CardHeader
            ...
            actions={
              <Link to="/exceptions">
                <Button variant="ghost" size="sm">Open queue</Button>
              </Link>
            }
          />
          <CardContent>
            {exceptionsQuery.isLoading || !exceptionsQuery.data ? (
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 border-t border-slate-100 pt-2">
                  {(Object.entries(exceptionsQuery.data.bySeverity) as [string, number][]).map(([severity, count]) => (
          ...
          {isLoading ? (
            ...
            <dl className="grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-1.5">
                ...
                <dd className="text-right font-medium text-slate-800">v{summary.run.modelConfigurationVersion ?? '—'}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-1.5">
                <dt className="text-slate-500">Scenario set</dt>
                <dd className="text-right font-medium text-slate-800">v{summary.run.scenarioSetVersion ?? '—'}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-1.5">
                <dt className="text-slate-500">Reporting date</dt>
                <dd className="text-right font-medium text-slate-800">{formatDate(summary.reportingDate)}</dd>
              </div>
              <div className="flex justify-between gap-4 border-b border-slate-100 pb-1.5">
            ...
            <p className="text-sm text-slate-500">No completed run yet, so there is no lineage to report.</p>
          )}
          <p className="mt-3 flex items-start gap-2 text-[11px] leading-relaxed text-slate-500">
            These versions are frozen on the run. Editing an assumption creates a new version and cannot change a
