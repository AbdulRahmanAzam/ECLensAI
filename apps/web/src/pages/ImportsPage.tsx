...
import { AlertTriangle, CheckCircle2, FileUp, Info, RefreshCw, SlidersHorizontal } from 'lucide-react';
import type {
  ImportBatchRecord,
  ImportIssuesRecord,
...
function BatchWorkflow({ batchPublicId }: { batchPublicId: string }) {
  ...
  const preview = useQuery({
    ...
    refetchInterval: (query) => (isInFlight(query.state.data?.batch.status ?? 'QUEUED') ? 1500 : false),
  });
  ...
  const applyMapping = useMutation({
    ...
    onError: (error) => push('error', 'Mapping rejected', messageOf(error, 'The mapping could not be applied.')),
  });
  ...
  const commit = useMutation({
    ...
    onError: (error) => push('error', 'Commit rejected', messageOf(error, 'The batch could not be committed.')),
  });
  ...
  const stageAiSuggestions = (suggestions: ImportMappingSuggestion[]) => {
    ...
    const shown = changes
      .slice(0, 4)
      .map((change) => `${change.field}: ${change.from === '' ? 'unmapped' : change.from} → ${change.to}`);
  ...
  return (
    <div className="space-y-5">
      {inFlight ? (
        <p className="flex items-center gap-2 rounded-md border border-navy-200 bg-navy-50 px-3 py-2 text-xs text-navy-800">
          <RefreshCw className="h-3.5 w-3.5 animate-spin" />
          {batch.status === 'COMMITTED' ? 'Writing the snapshot to the ledger…' : 'Validating rows…'} This panel
          refreshes itself.
        </p>
      ) : null}

      <section>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Import summary</h3>
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs sm:grid-cols-4">
          <div>
            <dt className="text-slate-500">Rows read</dt>
            <dd className="tabular-nums font-semibold text-slate-900">{formatNumber(batch.rowCount)}</dd>
          </div>
          <div>
            <dt className="text-slate-500">Valid</dt>
            <dd className="tabular-nums font-semibold text-emerald-700">{formatNumber(batch.validRowCount)}</dd>
          </div>
          <div>
        ...
        {batch.truncated ? (
          <p className="mt-1.5 flex items-start gap-1.5 text-[11px] text-amber-700">
            ...
            Only the first rows were parsed; {formatNumber(batch.rowsSkippedByTruncation)} were skipped by the
            bounded-memory reader. Split the file to import the rest.
          </p>
        ) : null}
        {batch.unmappedHeaders.length > 0 ? (
          <p className="mt-1.5 text-[11px] text-slate-500">
            Columns in the file that no field claims: <code>{batch.unmappedHeaders.join(', ')}</code>
          </p>
        ) : null}
      </section>
      ...
      <section>
        ...
        <p className="mb-2 text-[11px] leading-relaxed text-slate-500">
          The suggestions are deterministic string scoring against the canonical field labels and aliases — no model
          inference — so the same file always proposes the same mapping. Low-confidence guesses are marked and need
          your confirmation.
        </p>
        ...
        */}
        ...
        <div className="mt-3">
          <Select
            ...
            value={normalization}
            disabled={!can('import:map') || inFlight}
            onChange={(event) => setNormalization(event.target.value as PercentageNormalizationMode)}
        ...
        {missingRequired.length > 0 ? (
          <p className="mt-2 flex items-start gap-1.5 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-[11px] text-red-700">
            <AlertTriangle className="mt-0.5 h-3 w-3 shrink-0" />
            Required and still unmapped: <code>{missingRequired.join(', ')}</code>. The batch will fail validation
        ...
        {!can('import:map') ? (
          <p className="mt-1.5 text-[11px] text-slate-400">Your role cannot change a column mapping.</p>
        ) : null}
      </section>
            ...
            <tbody className="divide-y divide-slate-100">
              {previewRecord.previewRows.map((row) => (
                <tr key={row.rowNumber}>
                  <td className="whitespace-nowrap px-2 py-1.5 tabular-nums text-slate-400">{row.rowNumber}</td>
                  {row.cells.map((cell, index) => (
                    <td key={`${row.rowNumber}-${index}`} className="whitespace-nowrap px-2 py-1.5 text-slate-700">
                      {cell}
                    </td>
                  ))}
                </tr>
      ...
      <section>
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">
            ...
            <span className="ml-1 normal-case text-slate-400">
              ({issuesRecord?.issues.length ?? 0} total, {issuesRecord?.quarantinedRows.length ?? 0} row(s)
              quarantined)
            </span>
          </h3>
        ...
        {issues.isError ? (
          <ErrorState title="The issue report could not be loaded" onRetry={() => void issues.refetch()} />
        ) : issues.isLoading ? (
            ...
            <table className="w-full text-left text-[11px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {visibleIssues.map((issue, index) => (
                  <tr key={`${issue.rowNumber}-${issue.field}-${issue.issueCode}-${index}`} className="align-top">
                    ...
                    <td className="max-w-[10rem] truncate px-2 py-1.5 text-slate-700" title={issue.rawValue}>
                      {issue.rawValue === '' ? <span className="text-slate-300">(empty)</span> : issue.rawValue}
                    </td>
                    <td className="whitespace-nowrap px-2 py-1.5">
                      <Badge tone={SEVERITY_TONE[issue.severity]}>{issue.severity.toLowerCase()}</Badge>
        ...
        <p className="mt-1.5 flex items-start gap-1.5 text-[11px] leading-relaxed text-slate-500">
          ...
          Ambiguous values are never repaired silently. The raw cell is shown exactly as it was uploaded, and the
          downloadable error report on the Reports &amp; Exports screen carries the same rows for sign-off.
        </p>
      </section>

      <section className="rounded-md border border-slate-200 p-3">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Commit to a snapshot</h3>
        {batch.status === 'IMPORTED' ? (
          <p className="flex items-start gap-2 text-xs text-emerald-800">
            <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            Committed {batch.committedAt ? formatDateTime(batch.committedAt) : ''} by {batch.committedBy ?? '—'} as
            snapshot <code className="mx-1">{batch.snapshotLabel ?? '—'}</code>. A snapshot is immutable: it is the
            input version every later calculation cites.
          </p>
        ) : (
          <>
            ...
            {batch.status !== 'VALIDATED' ? (
              <p className="mt-1.5 text-[11px] text-slate-500">
                Only a <code>VALIDATED</code> batch can be committed — this one is <code>{batch.status}</code>.
...
export function ImportsPage() {
  ...
  const upload = useMutation({
    ...
    onError: (error) => push('error', 'Upload rejected', messageOf(error, 'The file could not be processed.')),
  });
  ...
  const columns = [
    ...
    column.accessor('uploadedAt', {
      ...
      enableSorting: false,
      cell: (info) => <span className="whitespace-nowrap text-slate-600">{formatDateTime(info.getValue())}</span>,
    }),
    ...
    column.accessor('validRowCount', {
      header: 'Valid',
      enableSorting: false,
      cell: (info) => <span className="tabular-nums text-emerald-700">{formatNumber(info.getValue())}</span>,
    }),
    ...
    <div>
      ...
      {!can('import:upload') ? (
        <p className="mb-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          Your role can read import history but cannot upload. Ask an admin or a risk analyst to run the ingestion.
        </p>
      ) : null}
